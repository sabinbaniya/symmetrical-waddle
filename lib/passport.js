...
const findOrCreateUser = async (profile, method) => {
    let user = await User.findOne({
        $or: [{ steamid: profile.steamid }, { email: profile.email }],
    });

    if (!user) {
        // Create new user
        // Note: Logic for new user creation might need adjustment based on exact business rules (e.g., initial balance)
        user = new User({
            username: profile.username,
            steamid: profile.steamid || ``, // Fallback if not steam login
            email: profile.email,
        ...
        // Link accounts or update info if needed
