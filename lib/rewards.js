...
export default class Rewards {
    static async getRewards(steamid, session = null) {
        const rewards = await rewardsDB
            .findOne({ steamid }, { _id: 0, history: 1 }, { session })
            .lean();
        return rewards?.history || {};
    }

    static async getFreeCases(steamid) {
        const rewards = await rewardsDB.findOne({ steamid }).lean();
    ...
    static async claimRakebackRewards(steamid, type) {
        // Check if user is eligible for the reward
        const depositAmount = await Stats.getDepositAmount(steamid);
        ...
        const rewards = await rewardsDB.findOne({ steamid });
        ...
        // Update rewards date
        await rewardsDB.updateOne({ steamid }, { history });
        await userDB.updateOne({ steamid }, { $inc: { balance: rewardAmount } });
    ...
    static async promoCode(user, code) {
        ...
        if (promoCode.usedBy.includes(steamid) || promoCode.usedBy.includes(secondaryID))
        ...
        await promoCodesDB.updateOne({ code }, { $push: { usedBy: steamid } });

        // Update user balance
        await userDB.updateOne({ steamid }, { $inc: { balance: promoCode.prize } });

        return { status: true, prize: promoCode.prize };
    }

    static async dailyCases(steamid, caseLevel, session = null) {
        ...
        const rewards = await Rewards.getRewards(steamid, session);
        const levels = (
            await casesDB.find(
                {
                    id: { $regex: /^level-\d+$/ },
        ...
        /*const user = await userDB.findOne({ steamid }, { _id: 0, experience: 1 }).lean();
        const userLevel = Auth.expToLevel(user.experience);
        if (userLevel < caseLevel) return { status: false, message: "Insufficient level" };

        const rewards = await Rewards.getRewards(steamid);*/
    }

    static async freeCases(steamid, update = true, session = null) {
        // Check if user level is sufficient
        const rewards = await rewardsDB
            .findOne(
                { steamid },
    ...
    static async getDepositBonus(steamid) {
        return (
            (await rewardsDB.findOne({ steamid }, { _id: 0, depositBonus: 1 }))?.depositBonus ||
            false
        );
    }

    static async useDepositBonus(steamid) {
        await rewardsDB.updateOne(
            { steamid },
            {
