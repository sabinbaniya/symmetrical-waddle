...
| `category` | `string` | `null`    | Optional filter by case category (e.g., `"community"`, `"official"`)                                                                   |
...
```bash
curl http://localhost:4000/cases?type=all
```
...
| `category` | `string` | `null`          | Optional filter by case category                                                    |
...
```bash
curl "http://localhost:4000/cases/pagination?page=1&limit=5&type=cs2&search=legendary&sort=Price%20Descending"
```
...
```bash
curl http://localhost:4000/cases/free
```
...
```bash
curl http://localhost:4000/cases/legendary-case
```
...
```typescript
{
    ...
    category?: string; // Optional category (defaults to "community")
...
```bash
curl -X POST http://localhost:4000/cases/create \
  -H "Content-Type: application/json" \
  -H "Cookie: connect.sid=s:..." \
  -d '{
    "name": "My Custom Case",
    "category": "community",
    "image": "https://example.com/case-image.png",
    "items": [
      {
        "marketHashName": "AK-47 | Redline (Field-Tested)",
        "percentage": 60
...
```typescript
{
  ...
  category: string;        // Case category (e.g., "community", "official")
...
```javascript
const response = await fetch("http://localhost:4000/cases?type=all");
...
```javascript
const response = await fetch("http://localhost:4000/cases/create", {
    method: "POST",
    headers: {
...
const response = await fetch(`http://localhost:4000/cases/pagination?${params}`);
